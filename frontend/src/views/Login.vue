		...
		<ion-content class="ion-padding">
			<div class="flex h-screen w-screen flex-col justify-center bg-white">
				<div class="flex flex-col mx-auto gap-3 items-center">
					...
					<div class="text-3xl font-semibold text-gray-900 text-center">Login to HR Mobile</div>
				</div>
			...
			<Dialog v-model="resetPassword.showDialog">
				...
				<template #actions>
					<a class="inline-flex items-center justify-center gap-2 transition-colors focus:outline-none text-white bg-gray-900 hover:bg-gray-800 active:bg-gray-700 focus-visible:ring focus-visible:ring-gray-400 h-7 text-base px-2 rounded"
						:href="resetPassword.link" target="_blank">
			...
			<Dialog v-model="otp.showDialog">
				...
				<template #body-content>
					...
					<form class="flex flex-col space-y-4" @submit.prevent="submit">
						<Input label="OTP Code" type="text" placeholder="000000" v-model="otp.code"
							autocomplete="one-time-code" />
						<ErrorMessage :message="errorMessage" />
						<Button :loading="session.otp.loading" variant="solid"
							class="disabled:bg-gray-700 disabled:text-white !mt-6">
							Verify
						</Button>
					</form>
